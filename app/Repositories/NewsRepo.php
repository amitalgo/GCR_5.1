...
class NewsRepo
{
    ...
    public function addNews($data)
    {
        ...
        $news->setUrlSlug(str_slug($data->get('title'),'-'));
        ...
        $news->setDeleted(0);
            $news->setCreatedAt(new \DateTime($data->get('newsdate')));
        ...
        //     $pageNews->setCreatedAt(new \DateTime());
        ...
        if($image_url){
            ...
            if($mulImage){
                foreach($mulImage as $mul)
                {
                    ...
                    $newsAttach->setCreatedAt(new \Datetime());
                    $newsAttach->setUpdatedAt(new \DateTime());
    ...
    {
        return $this->em->getRepository(News::class)->findBy(['deleted' => 0,'isActive'=>1],['createdAt'=>'ASC']);
    }
    ...
    public function getActiveNewsBySlug($slug)
    {
          return $this->em->getRepository(News::class)->findOneBy(['urlSlug'=>$slug]);
    }
