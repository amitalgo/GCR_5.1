...
//                    echo $route->getName();
                    $routesArray[substr($route->getName(),strpos($route->getName(), ".")+1)] = ["pageName"=>substr($route->getName(),strpos($route->getName(), ".")+1)];
                }
            }
        }

        foreach($routesArray as $key => $router){
            $routesJson[] = $router['pageName'];
//            foreach($router as $route){
//               $routesJson[] = $route['pageName'];
//            }
        }
