...
class NewsController extends Controller
{
    ...
    public function list(Request $request,$id)
    {
        ...
        $new = $this->newsService->getActiveNewsBySlug($id);
        $sortedData=$this->getDateWiseNews($news);
        $ads = $this->adService->getAdsByPage($this->page_id,2);
    ...
    public function getDateWiseNews($news)
    {
        ...
        {
            $timestamp = strtotime($element->getCreatedAt()->format('Y-m-d H:i:s'));
...
//                $sortedData[$date] = array($element);
                $sortedData[$date][]=array("id"=>$element->getId(),"updateAt"=>date("d F Y",$timestamp),"heading"=>$element->getNewsHeading(),"url"=>$element->getUrlSlug());
